...
class DuckDBConn(DatabaseFoundation):
    ...
    def _validate_args(self, args):
        ...
        """
        if (
                self._file_name is None and self._memory is None
        ) or (
                self._file_name is not None and self._memory is not None):
    ...
    def connect(self):
        ...
        try:
            if self._file_name:
                ...
                raise DBConnectionError('Ensure file name and memory params are define properly')

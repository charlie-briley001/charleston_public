...
4. `CREATE TABLE IF NOT EXISTS vehicles`
5. Insert all vehicle records and close the connection
...
- **Python 3.11+**
- **Windows** — the API key is read from Windows Credential Manager via `win32cred`. Linux/macOS would require replacing `get_auth` with a cross-platform keyring library.
- MBTA V3 API key stored in Windows Credential Manager
